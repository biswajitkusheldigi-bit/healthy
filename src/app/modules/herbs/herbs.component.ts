...
export class HerbsComponent implements OnInit {
    ...
    switch (this.pageType) {
      case 'herbs': {
        ...
        if (this.searchTerm && this.searchTerm.trim() !== '') {
          console.log('THIS RAN!', this.filterUrl.page);
          req = this.productService.getHerbsSearchResults(this.searchTerm, {
            page: this.filterUrl.page,
            limit: 12,
          });
        } else {
          payload.slug = 'herbs';
  ...
  onLoadMore() {
    this.filterUrl.page++;
    if (this.pageType === 'herbs') {
      // Call the same API used for herbs listing
      this.getProducts(false);
    } else {
      // For all other types, use generic filter logic
      this.onFilterChange({});
    }
  }
